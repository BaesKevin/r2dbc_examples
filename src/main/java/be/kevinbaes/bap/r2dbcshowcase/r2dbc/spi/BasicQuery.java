...
public class BasicQuery {
  ...
  private void run() {
    ...
    /******************
     * BASIC INSERT IN TRANSACTION
     ********************/

    Mono.from(connectionFactory.create())
        .flatMapMany(
            conn ->
                Mono.from(conn.beginTransaction()) // 1.
                    .thenMany(
                        Flux.from(
                            conn
                                .createStatement("INSERT into GOAL values(1, 'new goal')")
                                .execute()
                        ))
                    .delayUntil(p -> Mono.from(conn.commitTransaction())) // 2.
                    .concatWith(Mono.from(conn.close()).then(Mono.empty())) // 3.
                    .onErrorResume(e ->
                        Mono.from(conn.rollbackTransaction()) // 4
